...
using System.Text;
using static geckou.GeckoUException;
    ...
    public class GeckoUCore
    {
        ...
        public bool Connect()
        {
            ...
            catch (IOException)
            {
                ...
                this.Disconnect();
                throw new GeckoUException(ETCPErrorCode.noTCPGeckoFound);
            }
        ...
        public UInt64 CallFunction64(uint address, params uint[] args)
        {
            ...
            if (RawCommand(GeckoUCommands.cmd_rpc) != FTDICommand.CMD_OK)
                ...
                MessageBox.Show("Your Wii U have crash or you are disconnected to MamiesMod V4 !", "MamiesMod V4");
        ...
        public uint MixMillV2(uint baseval, decimal val)
        {
            decimal value = baseval + (0x10000 * val);
            return (uint)value;
        }

        public uint MixMillX4V2(uint baseval, decimal val)
        {
            decimal value = baseval + (0x40000 * val);
            return (uint)value;
        }

        public uint Mix4_V2(uint baseval, decimal val)
        {
            decimal value = baseval + (0x4 * val);
            return (uint)value;
        }
        ...
        public void clearString2(uint startAddress, uint endAddress)
        {
            ...
            {
                WriteUInt(startAddress + Convert.ToUInt32(x), 0x00000000);
                x = x + 4;
            }
        }
        ...
        {
            byte[] resulte = new byte[input.Length / 2];
            int cur = 0;

            for (int i = 0; i < input.Length; i = i + 2)
            {
                string w = input.Substring(i, 2);
                resulte[cur] = Convert.ToByte(w, 16);
                cur++;
            }
            WriteBytes(adress, resulte);
        }
    }
}
